...
`@jtalk22/slack-mcp@3.0.0` hardens hosted HTTP defaults while preserving the local operator workflows (`stdio`, `web`).

Hosted-only breaking scope: existing hosted deployments must configure auth and CORS env vars before upgrade.

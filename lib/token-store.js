...
// Freshly extracted tokens whose persistence failed (e.g. locked Keychain in
// keychain-only mode). Kept in-process so an immediate retry uses the working
// tokens instead of re-reading the stale persisted credentials that just
// failed auth. Cleared by the next successful save.
let unpersistedTokens = null;

export function _clearUnpersistedTokensForTests() {
  unpersistedTokens = null;
}
...
/**
 * Remove the plaintext token file, failing loudly if it survives. Called
 * only after credentials are verified in the Keychain: a silent failure
 * here would leave plaintext on disk while every status surface claims
 * keychain-only, and would re-trigger migration on every subsequent load.
 */
function removePlaintextTokenFileOrThrow() {
  try { unlinkSync(TOKEN_FILE); } catch (e) { /* verified below */ }
  if (existsSync(TOKEN_FILE)) {
    const err = new Error(
      `keychain-only mode: credentials are safely in the Keychain, but the plaintext file ${TOKEN_FILE} could not be removed. ` +
      "Delete it manually to finish the cleanup."
    );
...
function getStoredTokens() {
  ...
  // A refresh that extracted working tokens but failed to persist them
  // (locked Keychain) must not fall back to the stale stored credentials
  // that just failed auth — serve the fresh in-memory pair until a save
  // succeeds.
  if (unpersistedTokens) {
    return unpersistedTokens;
  }
...
export function saveTokens(token, cookie) {
  try {
    saveTokensToStores(token, cookie);
    unpersistedTokens = null;
  } catch (e) {
    if (e.code === "plaintext_removal_failed") {
      // Credentials reached the Keychain; only the plaintext cleanup failed.
      unpersistedTokens = null;
    } else {
      unpersistedTokens = {
        token,
...
function saveTokensToStores(token, cookie) {
  const mode = getStorageMode();
  ...
  if (mode === "keychain-only") {
    ...
    // strictly after the verified Keychain write above, and a file that
    // cannot be removed is an error, not a silent leftover.
    if (existsSync(TOKEN_FILE)) {
      removePlaintextTokenFileOrThrow();
    }
    return;

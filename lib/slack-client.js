  ...
  if (hasKnownAge && tokenAge > TOKEN_WARNING_AGE && Date.now() - lastRefreshAttempt > REFRESH_COOLDOWN) {
    ...
    if (newTokens) {
      try {
        ...
        // Keychain-only persistence failure: keep the fresh tokens for this
        // process; the next health check re-extracts.
    ...
    if ((data.error === "invalid_auth" || data.error === "token_expired") && retryOnAuthFail) {
      ...
      if (chromeTokens) {
        try {
          ...
          logger.error?.(`Refreshed tokens could not be persisted: ${persistError.message}`);
        }
        saveAutoHealTelemetry({ attemptAt, error: null });
        // Retry the request

  ...
  if (hasKnownAge && tokenAge > TOKEN_WARNING_AGE && Date.now() - lastRefreshAttempt > REFRESH_COOLDOWN) {
    ...
    if (newTokens) {
      ...
      try {
        ...
        // The fresh tokens stay usable in-process via the token store's
        // memory fallback, but a failed persist is not a clean heal —
        // record it so the stuck detector trips if it stays broken.
        persistError = e;
        logger.error?.(`Refreshed tokens could not be persisted: ${e.message}`);
      }
      saveAutoHealTelemetry({ attemptAt, error: persistError ? "token_persist_failed" : null });
    ...
    if ((data.error === "invalid_auth" || data.error === "token_expired") && retryOnAuthFail) {
      ...
      if (chromeTokens) {
        ...
        try {
          ...
          // The retry still works: loadTokens serves the fresh pair from
          // the token store's memory fallback. But don't record a clean
          // heal when persistence failed.
          persistError = e;
          logger.error?.(`Refreshed tokens could not be persisted: ${e.message}`);
        }
        saveAutoHealTelemetry({ attemptAt, error: persistError ? "token_persist_failed" : null });

...
<head>
  ...
  <style>
    ...
    /* ═══════════════════════════════════════════════════════════════
       ...
       Inside the app-window fiction the SYSTEM register applies:
       sys-green success, sys-blue links, clay assistant accent,
       real traffic lights. Estate ink/paper carry the ground.
       ═══════════════════════════════════════════════════════════════ */
    :root {
      ...
      /* Accents */
      --link-color: var(--sys-blue);
      --code-bg: var(--surface);
      --code-text: var(--paper);
      --success-color: var(--sys-green);
      --warning-color: var(--signal);

      /* Shadows */
      --shadow-lg: 0 30px 60px -18px rgba(0, 0, 0, 0.72);
    ...
    * {
    ...
    body {
      ...
      background: linear-gradient(135deg, var(--desk-1), var(--desk-2), var(--desk-3));
    ...
    .skeleton-avatar {
      ...
      height: 28px;
      border-radius: 50%;
      background: linear-gradient(90deg, var(--window-border) 25%, #2e2e34 50%, var(--window-border) 75%);
    ...
    .message.user .message-avatar {
      background: linear-gradient(135deg, #2a2a2e 0%, #3c3c42 100%);
      color: white;
      font-weight: 600;
      font-size: 12px;
    }
    ...
    .message.claude .message-avatar {
      background: linear-gradient(135deg, var(--clay) 0%, #b85f41 100%);
      color: white;
      font-weight: 600;
      font-size: 12px;
    }
    ...
    .message.claude .message-sender { color: var(--clay); }
    ...
    /* Title / closing cards — flat ink covers; one accent rule each */
    ...
    .title-card::before { background: var(--stamp); }
    .closing-card::before { background: var(--sys-green); }
    .title-card {
      position: absolute;
      inset: 0;
    ...
    .title-card h1 .accent-time { color: var(--stamp); }
    .title-card .chan { color: var(--signal); font-weight: 600; }
    ...
    /* Hero line gets a touch of scale on reveal (tasteful, not garnish) */
    @keyframes titleHeroReveal {
      from { opacity: 0; transform: translateY(10px) scale(0.985); }
      to   { opacity: 1; transform: translateY(0) scale(1); }
    }
    .title-card.visible h1.title-line { animation: titleHeroReveal 0.62s cubic-bezier(0.22, 1, 0.36, 1) forwards; }
    ...
    /* Closing Card — the poster payoff (47 → 0) */
    .closing-card {
      position: absolute;
      inset: 0;
      top: 32px;
    ...
    /* 47 → 0 hero — mirrors docs/images/demo-poster.png */
    .closing-tally {
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    .closing-tally .from { color: var(--stamp); }
    .closing-tally .to { color: var(--sys-green); }
    .closing-tally .arrow {
      color: var(--text-muted);
      font-size: 0.52em;
      transform: translateY(-0.06em);
    }
    .closing-card.visible .closing-tally .from { animation: tallyPop 0.6s cubic-bezier(0.22, 1, 0.36, 1) 0.15s both; }
    .closing-card.visible .closing-tally .arrow { animation: tallyPop 0.6s cubic-bezier(0.22, 1, 0.36, 1) 0.42s both; }
    .closing-card.visible .closing-tally .to { animation: tallyPop 0.72s cubic-bezier(0.34, 1.56, 0.64, 1) 0.62s both; }
    @keyframes tallyPop {
      from { opacity: 0; transform: translateY(0.14em) scale(0.7); }
      to   { opacity: 1; transform: translateY(0) scale(1); }
    }
...
{{DEMO_LINKS}}
  ...
  <div class="claude-window">
    ...
    <div class="title-card" id="titleCard">
      <div class="title-line" style="margin-bottom: 20px;">
        <img src="data:image/svg+xml;base64," alt="" style="width: 44px; height: 44px; border-radius: 10px;">
      </div>
      <h1 class="title-line">It's Monday,<br><span class="accent-time">9:07 AM.</span></h1>
      <p class="title-line" style="font-size: 19px; color: var(--text-secondary); line-height: 1.6; max-width: 30ch;">A jammed printer. Nobody knows the PIN.<br>The guy who did left six months ago.</p>
      <p class="title-line" style="margin-top: 26px; font-size: 18px; color: var(--text-primary); font-weight: 500;">His message is still in <span class="chan">#facilities</span>.</p>
      <p class="title-line title-version" style="margin-top: 20px; font-size: 13px; color: var(--text-muted); letter-spacing: 0.01em;">Slack MCP Server &nbsp;·&nbsp; No OAuth &nbsp;·&nbsp; One command</p>
    </div>
    ...
    <div class="closing-card" id="closingCard">
      <div class="closing-tally" aria-label="47 unreads to 0">
        <span class="from">47</span>
        <span class="arrow" aria-hidden="true">→</span>
        <span class="to">0</span>
      </div>
      <h2 style="margin-bottom: 12px;">You never opened Slack.</h2>
      <p style="font-size: 14px; color: var(--text-muted); margin-bottom: 28px;">47 unreads, cleared. And the printer PIN is 4729.</p>
  ...
  <script>
    ...
    async function runScenario(scenarioId) {
      ...
      // Narrative clock: the story opens Monday 9:07 AM; each scenario advances
      // two minutes. Deterministic recordings, on-script timestamps.
      window.__narrativeStep = (window.__narrativeStep ?? -1) + 1;
      const now = new Date(2026, 0, 5, 9, 7 + window.__narrativeStep * 2);

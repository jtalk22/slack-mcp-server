...
### Added
- **Keychain-only credential storage** (`SLACK_MCP_TOKEN_STORAGE=keychain-only`, macOS) — credentials live exclusively in the macOS Keychain and no plaintext token file is ever written (#162). `--setup`, `slack_refresh_tokens`, and automatic refresh work unchanged. An existing `~/.slack-mcp-tokens.json` is migrated into the Keychain on first load and removed only after both entries verify by read-back; a failed migration leaves the file untouched and reports `keychain_migration_failed`, and a file that cannot be removed after a verified Keychain write fails loudly (`plaintext_removal_failed`) instead of being silently left behind. Keychain writes are verified and fail loudly (`keychain_write_failed`) instead of falling back to plaintext. Non-secret bookkeeping (token timestamp, auto-heal telemetry) moves to `~/.slack-mcp-meta.json` so `slack_token_status` age reporting and stuck-detection keep working.
- **In-memory fallback for failed persists** — when a refresh extracts working tokens but cannot persist them (e.g. locked Keychain), the fresh pair stays available in-process (`source: "memory"`) until a save succeeds, so retries do not fall back to the stale credentials that just failed auth. Auto-heal telemetry records `token_persist_failed` for these cycles instead of a clean heal.
...
- **Setup wizard storage prompt (macOS)** — `--setup` asks where credentials should live (token file + Keychain backup, or Keychain only) and persists the choice in `~/.slack-mcp-meta.json`, so the MCP server, CLI, and any LaunchAgent follow it without plumbing an env var into each client config. `SLACK_MCP_TOKEN_STORAGE` overrides the persisted choice when set. The prompt re-asks on unrecognized input rather than defaulting a typo to plaintext storage, save failures surface a friendly Keychain error instead of a stack trace, and setup refuses keychain-only mode outright on platforms without a Keychain — before collecting any credentials.
